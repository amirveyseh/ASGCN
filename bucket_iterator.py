...
class BucketIterator_Bert(object):
    ...
    def pad_data(self, batch_data):
        ...
        batch_bert_in_indices = []
        batch_bert_out_indices = []
        max_len = max([len(t[self.sort_key]) for t in batch_data])
        max_len_bert = max([len(t['bert_in_indices']) for t in batch_data])
        for item in batch_data:
            text_indices, context_indices, aspect_indices, left_indices, polarity, dependency_graph, bert_in_indices, bert_out_indices = \
                item['text_indices'], item['context_indices'], item['aspect_indices'], item['left_indices'], \
                item['polarity'], item['dependency_graph'], item['bert_in_indices'], item['bert_out_indices']
            ...
            batch_dependency_graph.append(numpy.pad(dependency_graph, \
                                                    ...
                                                    'constant'))
            # *********** for BERT *************
            bert_in_padding = [0] * (max_len_bert - len(bert_in_indices))
            bert_out_padding = [0] * (max_len - len(bert_out_indices))
            batch_bert_in_indices.append(bert_in_indices + bert_in_padding)
            batch_bert_out_indices.append(bert_out_indices + bert_out_padding)
            # **********************************
        return { \
            ...
            'bert_in_indices': torch.tensor(batch_bert_in_indices),
            'bert_out_indices': torch.tensor(batch_bert_out_indices)
        }
